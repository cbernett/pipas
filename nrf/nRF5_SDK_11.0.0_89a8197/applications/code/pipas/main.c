...
#define DEVICE_NAME                     "Pipas1000"                               /**< Name of device. Will be included in the advertising data. */
...
{
		//uart_send("Flash event.\n");
		//uart_send((char*)&m_rx_buf);
    //uint8_t d = m_rx_buf[0];
		spi_xfer_done = true;
		char* d = "spi done.\n";
		ble_nus_string_send(&m_nus, d, strlen(d));
		//char s[20];
		//sprintf(s, "%d:%d:%d:%d\n", output_buffer[10], output_buffer[11], output_buffer[12], output_buffer[13]);
		//uart_send(s);
